...
def game(call):
    ...
    if call.data.startswith('user'):
        ...
        if (str(buttons)).count('X') == 10:
            bot.send_message(call.message.chat.id, 'Игра окончена. Вы проиграли, это была хорошая игра, а вы достойнам противником! Не растраивайтесь и поробуйте еще раз, в следующий раз вы победите! Чтобы сыграть еще раз, нажмите /restart')
